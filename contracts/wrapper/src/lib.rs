    ...
    fn calc_shares_for_assets(
        ...
        if let Ok(vault_state) = Self::read_vault_state(env) {
            if vault_state.exchange_rate > 0 {
                return math::mul_div_down(scaled_assets, math::SCALING_FACTOR, vault_state.exchange_rate);
            }
        }
...
impl WrapperContract {
    ...
    pub fn unwrap(env: Env, caller: Address, wrapped_amount: i128) -> Result<(), WrapperError> {
        ...
        let underlying_decimals = underlying_client.decimals();
        let underlying_amount = Self::calc_assets_for_shares(&env, underlying_decimals, wrapped_amount)
            .unwrap_or_else(|| {
