...
//! - **`instance()`** ΓÇö Contract-wide singleton state. Used for admin address, admin

//!   pool, threshold, proposals, and both proposal ID counters.

//! - **`persistent()`** ΓÇö Per-key state with independent TTL. Used for role
...
//! - [`has_admin`] returns `false` when no admin is stored ΓÇö callers use this to
...
//!   (`GAAAAΓÇªWHF`) via `require_non_zero_address` ΓÇö the all-zero ed25519 public
...
//!   is considered to have every role. This simplifies authorization ΓÇö admins
...
//! - An already-`Executed` proposal cannot be cancelled ΓÇö upgrades are
...
//!   multi-step operations (e.g., create ΓåÆ approve ΓåÆ execute proposal) should
...
/// under `AdminKey::RoleMask(Address)`; each variant maps to a single bit ΓÇö

/// `Admin` = `1 << 0` (1), `Minter` = `1 << 1` (2), `SuperAdmin` = `1 << 2`

/// (4), `Pauser` = `1 << 3` (8) ΓÇö see [`ROLE_BIT_ADMIN`], [`ROLE_BIT_MINTER`],
...
/// The SuperAdmin role constant ΓÇö can be imported as `SUPER_ADMIN_ROLE` for
...
/// The Minter role constant ΓÇö can be imported as `MINTER_ROLE` for
...
/// @custom:bitmask-value 1 ΓÇö the role bit used by [`Role::Admin`] in
...
/// @custom:bitmask-value 2 ΓÇö the role bit used by [`Role::Minter`] in
...
/// @custom:bitmask-value 4 ΓÇö the role bit used by [`Role::SuperAdmin`] in
...
/// @custom:bitmask-value 8 ΓÇö the role bit used by [`Role::Pauser`] in
...
///      Storage slots: `AdminKey::Admin` (instance) and `AdminKey::RoleMask(admin)` (persistent) ΓÇö no overlap.
...
/// - `AdminKey::Admin` (instance) ΓåÆ `Address`

///

/// After migration:

/// - `AdminKey::Admin` (instance) ΓåÆ `Address` (unchanged)

/// - `AdminKey::SuperAdmin(address)` (persistent) ΓåÆ `true` (new entry)
...
/// - [`AdminError::UnauthorizedRole`] ΓÇö `caller` does not hold the `SuperAdmin` role.

/// - [`AdminError::InvalidAddress`] ΓÇö `address` is the canonical zero address.

/// - [`AdminError::InvalidRole`] ΓÇö `role` is not a recognized variant.
...
/// - [`AdminError::RoleNotHeld`] ΓÇö `address` does not currently hold `role`.
...
pub fn revoke_role(
    ...
    // #426 ΓÇô parameter validation: reject unknown role variants and the zero address.
...
/// Timelock guard ΓÇö reverts while the mandatory delay is still running.
...
/// `Approved` in the same call ΓÇö mirroring the existing [`approve_proposal`]
...
/// allowlist: the contract admin explicitly records a hash here ΓÇö typically

/// right after uploading it ΓÇö before any upgrade proposal is allowed to
...
mod tests {
    ...
    fn test_non_privileged_caller_cannot_grant_pauser() {
        ...
        // caller has no role ΓÇö should be rejected
    ...
    fn test_non_super_admin_cannot_grant_pauser() {
        ...
        // A non-privileged caller cannot grant the Pauser role ΓÇö the call is
    ...
    /// #765 ΓÇô SuperAdmin absolute privileges: a dedicated SuperAdmin can revoke any role.
    ...
    fn test_super_admin_can_revoke_any_role() {
        ...
        for role in roles {
            ...
            // 2ΓÇô3. SuperAdmin revokes the role from User A.
    ...
    /// #765 ΓÇô Error path: SuperAdmin receives RoleNotHeld when revoking a role User A never held.
    ...
    /// #765 ΓÇô Error path: callers without SuperAdmin cannot revoke any role.
    ...
    /// #765 ΓÇô Error path: a revoked SuperAdmin loses absolute revoke privileges.
    ...
    // ΓöÇΓöÇ #417: RoleGranted event ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ Role-mask mapping assignment ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ #405: init_storage ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ #426: revoke_role role-parameter validation ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ require_admin ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ require_upgrade_quorum_met (#656) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ approve_upgrade (#654) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ register_wasm_hash / require_valid_wasm_hash (#657) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ cancel_proposal (#662) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
