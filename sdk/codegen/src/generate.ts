...
import type { ContractAbi, ParsedFunc, ParsedStruct, ParsedUnion, ParsedEnum } from './parse-abi';

// ─── Type Mapping ─────────────────────────────────────────────────────────────

function scSpecTypeToTs(typeDef: xdr.ScSpecTypeDef): string {
...
function generateStructInterface(s: ParsedStruct): string {
...
function generateUnionType(u: ParsedUnion): string {
...
function generateEnumType(e: ParsedEnum): string {
...
function generateErrorEnum(errors: ContractAbi['errors'], contractName: string): string {
...
function generateMethod(fn: ParsedFunc): string {
  const doc = fn.doc
    ? `  /**\n   * ${fn.doc}\n   */\n`
    : '';
...
function generateClientInterface(funcs: ParsedFunc[], contractName: string): string {
...
export function generateBindings(abi: ContractAbi, contractName: string): string {
  const banner = [
    '// ─────────────────────────────────────────────────────────────────────────────',
  ...
  if (errorEnum) parts.push(errorEnum);
